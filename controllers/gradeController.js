...
const update = async (req, res) => {
  ...
  try {
    await Grades.replaceOne({ _id: id }, updateData, {
      $set: { lastModified: '$$NOW' },
    });
    updateData = await Grades.findOne({ _id: id });
    logger.info(`PUT /grade - ${id} - ${JSON.stringify(req.body)}`);
    res.send(`PUT /grade - ${id} - ${JSON.stringify(req.body)}`);
  } catch (error) {
...
const remove = async (req, res) => {
  ...
  try {
    let grades = await Grades.findOne({ _id: id });
    if (grades) {
      grades = new Grades(grades);
      await grades.deleteOne();
